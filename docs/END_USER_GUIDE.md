...
- A **📄 Project Report** section inside each query — click to expand, choose a **report scope**, a **report type**, then **Generate Report**:
  ...
  - **Status hygiene check:** Status Report and 1:1 Prep both check whether any of your top-priority Backlog items have a Jira comment within the last 14 days — a sign real work is happening even though the status was never updated. When that happens, the report names the item and suggests updating its status, so workload snapshots (this one and future ones) read accurately at a glance.
